...
import { ArrowLeft, Play, X, Image as ImageIcon } from 'lucide-react'; // Added Image icon
...
interface Project {
	...
	components?: string[];
	location?: string;
	videos?: string[]; // Array of video links (Google Drive, YouTube, etc.)
	images?: string[]; // Array of image links (Google Drive)
}
...
const getGoogleDriveThumbnailUrl = (url: string): string | null => {
	...
	if (fileIdMatch && fileIdMatch[1]) {
		const fileId = fileIdMatch[1];
		return `https://drive.google.com/thumbnail?id=${fileId}&sz=w1280-h720`; // Construct a thumbnail URL. sz=w640-h480 sets a reasonable size, we can use other values here
	}

	return null; // Couldn't extract file ID
};
...
const getYoutubeThumbnailUrl = (url: string): string | null => {
	...
	const getYouTubeId = (url: string): string | null => {
		...
		if (url.includes('youtube.com')) {
			...
			videoId = url.split('/').pop()?.split('?')[0] || null; // Handles youtu.be/VIDEO_ID?t=...
		}
		return videoId;
	};
	...
	if (videoId) {
		return `https://img.youtube.com/vi/${videoId}/maxresdefault.jpg`; // Use maxresdefault for best quality,  hqdefault.jpg is another option
	}
	return null;
};
...
const getThumbnailUrl = (url: string): string | null => {
	...
	); //Prioritize Google Drive Thumbnails
};
...
const getGoogleDriveImageUrl = (url: string): string | null => {
	...
	if (fileIdMatch && fileIdMatch[1]) {
		const fileId = fileIdMatch[1];
		return `https://lh3.googleusercontent.com/d/${fileId}`; //Direct image link using fileId
	}
...
const ProjectDetail = (): JSX.Element => {
	...
	const [isImageOpen, setIsImageOpen] = useState<boolean>(false); // State for image modal
	...
	return (
		<div className='min-h-screen bg-black text-white'>
			<div className='container mx-auto px-4 py-8 md:py-12'>
				<Link
					to='/'
					className='inline-flex items-center text-white hover:text-gray-300 mb-6 md:mb-8'
				>
					<ArrowLeft className='mr-2' size={16} /> Back to Home
				</Link>
									...
									>
										{thumbnailUrl ? (
											<img
												...
												sizes='(max-width: 640px) 100vw, (max-width: 768px) 50vw, 33vw' // Responsive image sizing
											/>
										) : (
								...
								>
									...
									<div className='w-full aspect-video bg-zinc-900 rounded-lg overflow-hidden shadow-xl'>
										<img
											src={selectedImage}
											alt='Project Image'
											className='object-contain w-full h-full' // Use object-contain for proper display in modal
										/>
									</div>
								</div>
							</motion.div>
